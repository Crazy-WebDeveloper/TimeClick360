...
exports.signup = async (req, res, next) => {
    ...
    if (EV == true) {
        ...
        var sdata = await BASECONTROLLER.data_save({ company_name: req.body.companyname, employee_number: req.body.numofemployee, email: req.body.email, telephone: req.body.telephone }, COMPANYMODEL);
        if (sdata) {
            var udata = await BASECONTROLLER.data_save({ id: sdata._id, first_name: req.body.firstName, last_name: req.body.lastName, email: req.body.email, telephone: req.body.telephone, password: password, role: "admin", allowed: true }, USERMODEL)
...
exports.signin = async (req, res, next) => {
    const password = await BASECONTROLLER.jwt_encode(req.body.password);
    var EV = await BASECONTROLLER.email_verify(req.body.email);
    var PV = await BASECONTROLLER.password_verify(req.body.email, password);
    var AV = await BASECONTROLLER.allowed_verify(req.body.email);
    ...
    if (!EV) {
        ...
        res.json({
            status: false,
            data: "You are not allowed yet! Please ask to admin!"
        })
    } else {
        var udata = await BASECONTROLLER.BfindOne(USERMODEL, {email: req.body.email});
